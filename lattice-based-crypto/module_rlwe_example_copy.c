...
void poly_add(poly *res, poly *a, const poly *b) {
    ...
    for (int i = 0; i < N; i++)
        res->coef[i] = (a->coef[i] + b->coef[i]) % Q;
}
...
void poly_sub2(poly *res, poly *a, const poly *b) {
    ...
    for (int i=0; i<N; i++) {
        if (a->coef[i] >= b->coef[i])
            res->coef[i] = (a->coef[i] - b->coef[i]);     // Q isn't need because coef of a is less than Q and coef of b is less than coef of a
        else
            res->coef[i] = (a->coef[i] + Q - b->coef[i]); // a->coef < b->coef < Q
    }
...
/*
    Result (polynomial) = polynomial * polynomial (polynomial in R_Q)
        poly_mul is schoolbook multiplication : O(N^2)
*/
void poly_mul(poly* res, const poly* a, const poly* b) {
    uint32_t tempcoef = 0;
    uint16_t temppoly[N * 2] = { 0, };
...
*/
static uint32_t load32_littleendian(const uint8_t x[4]) { // 23.1.26 15:13
    uint32_t res;
    res  = (uint32_t)x[0];
    res |= (uint32_t)x[1] << 8;
    res |= (uint32_t)x[2] << 16;
...
*/
static void cbd(poly *res, const uint8_t buf[ETA1*N/4]) { // (buf is random array)
    unsigned int i,j;
    uint32_t t,d;
    int16_t a,b;
...
void prf(uint8_t res[ETA1*N/4], uint8_t seed[32], uint8_t nonce) {
    ...
    for (int i=0; i<ETA1*N/4; i++) { // 
        count++;
        if ( (count % ((ETA1*N/4)/6)) == 1 ) // 6번마다 // 0번 -> 0번, ..., 192번 -> 32번
...
/* XOF function 
    seed0 seed1 seed2 ... seed31 -> res0 res1 res2 ... res512 my_xof, comment
*/
    ...
    for (int i=0; i<K; i++) { // 2
        for (int j=0; j<K; j++) {  // 2
            for (int k=0; k<N; k++) { // 256 --> 1024번
                ...
                res[i].vec[j].coef[k] = (uint16_t)(rand()%Q); // rand에 seed를 이용하여 균등 분포로 샘플링
            }
        }
    }
...
void samplematrix2(polyvec *res, uint8_t seed[32]) {
    ...
    for (int i=0; i<K; i++) {
        for (int j=0; j<K; j++) {
            ...
            l = 0;
            for (int k=0; k<N; k++) {
                res[i].vec[j].coef[k] = ((((uint16_t)buf[l])<<8) | ((uint16_t)(buf[l+1])&0x00ff)) % Q;
...
void poly_tomsg(uint8_t res[32], poly *mp) {
    ...
    for(i=0;i<N/8;i++) {
        res[i] = 0;
        for(j=0;j<8;j++) {
            t = ((((uint16_t)mp->coef[8*i+j] << 1) + Q/2)/Q) & 1; 
            res[i] |= t << j;
        }
    }
}
 ...
 * In Kyber :
        ...
        sk : s (768 bytes)
 */
void keygen(polyvec *b, uint8_t publicseed[32], polyvec *s) {
    polyvec A[K], e;
 ...
 */
void enc(polyvec *ct_u, poly* ct_v, uint8_t *m, polyvec *b, uint8_t publicseed[32], uint8_t coin[32]) {
 ...
 */
void dec(polyvec *ct_u, poly *ct_v, polyvec *s, uint8_t recovered[32]) {
    /* ct_v - s^T * ct_u */
    poly mp, stu;
...
void module_rlwe_test() {
    ...
    enc(&ct_u, &ct_v, m, &b, publicseed, coin);
...
void printmatrix(char *msg, polyvec vector[K]) {
    ...
    for (int k=0; k<K; k++ ) {
        for (int i=0; i<K; i++) {
            printf(" [ ");
            for (int j=0; j<N; j++) { 
                printf("%d ", vector[k].vec[i].coef[j]);
            }
            printf(" ] \n");
        }
    }
