...
class randombytes:
    ...
    def get_randomarray():
        return [random.randint(0, 256) for _ in range(SYMBYTES)]
...
class poly:
    ...
    def tobytes(p):
        ...
        # p03 p02 p01 p00    || p13 p12 p11 p10 (4비트씩)
        # 
...
class polyvec:
    ...
    """
        ...
        polyvec ct_u (1024-bytes) <- byte array C1 (640-bytes)
        
        ct_u <- Decompress_Q( Decode_10(C1), 10 )
    """
    @staticmethod
    def decompress(c1):
        res = polyvec()
...
class kyber512:
    ...
    def keygen(pk, sk):
        ...
        pk_temp = byte.concate(polyvec.tobytes(b), publicseed)
        for i in range(PUBLICKEYBYTES):
            pk[i] = pk_temp[i] 
        sk_temp = polyvec.tobytes(s) # -ETA1 ~ ETA1
    ...
    def enc(pk, m, coin, ct):
        ...
        b = polyvec.frombytes(pk[:PUBLICKEYBYTES]) # Decode_12(pk)
        
        publicseed = pk[POLYVECBYTES:]             # pk[768:800]

        AT.sample(publicseed, True)                # True means transpose
        r.sample(coin, nonce, ETA1); nonce += K    # Sample r,  Increase nonce
        e1.sample(coin, nonce, ETA2); nonce += K   # Sample e1, Increase nonce
        e2 = poly.sample(coin, nonce, ETA2)        # Sample e2, Increase nonce
        

        mp = poly.frommsg(m)                       # Decompress_Q( Decode_1(m), 1 ) 
        ct_u = AT * r + e1
        ct_v = b * r + e2 + mp
        
        
        ct1 = polyvec.compress(ct_u)               # Encode_d_u( Compress_q(ct_u, d_u) )
        ct2 = poly.compress(ct_v)                  # Encode_d_v( Compress_q(ct_v, d_v) ) ( 128-bytes array <- poly() )
        ct_temp = byte.concate(ct1, ct2)           # ct <- (c1 || c2)
        
        for i in range(len(ct_temp)):
            ct[i] = ct_temp[i]                     # ct <- (c1 || c2)
        
    # 3. Decryption part
    def dec(sk, ct, recovered):
        ct1 = ct[:POLYVECCOMPRESSEDBYTES]          # ct[0  :640]  (640-bytes)
        ct2 = ct[POLYVECCOMPRESSEDBYTES:]          # ct[640:768]  (128-bytes)

        ct_u = polyvec.decompress(ct1)
        ct_v = poly.decompress(ct2)
        
        s = polyvec.frombytes(sk)
        
        mp_witherror = ct_v - (s * ct_u)
        
        recovered_temp = poly.tomsg(mp_witherror)
